...
class TestPngOutputValidation:
    ...
    def _files_are_similar(self, file1: Path, file2: Path, tolerance: float = 0.1) -> bool:
    ...
    @pytest.mark.parametrize("svg_filename,expected_png_filename", [
        ("architecture_mermaid_0_07a67020.svg", "architecture_mermaid_0_07a67020.png"),
        ("class-design_mermaid_0_86b4976d.svg", "class-design_mermaid_0_86b4976d.png"),
        ("database-design_mermaid_0_3d6e6222.svg", "database-design_mermaid_0_3d6e6222.png"),
        ...
        ("project-plan_mermaid_0_336420ee.svg", "project-plan_mermaid_0_336420ee.png"),
        ("state-management_mermaid_0_5231eec1.svg", "state-management_mermaid_0_5231eec1.png"),
        ("system-overview_mermaid_0_93d92671.svg", "system-overview_mermaid_0_93d92671.png"),
        ("user-journey_mermaid_0_ccef6318.svg", "user-journey_mermaid_0_ccef6318.png"),
    ])
    ...
    def test_basic_svg_files_conversion(
        ...
        for svg_filename, expected_png_filename in test_cases:
            ...
            assert actual_png.stat().st_size > 1000, \
                f"Generated PNG too small for {svg_filename}: {actual_png.stat().st_size} bytes"
    ...
    def test_different_scale_output_sizes(
        ...
        assert scale_20_size > scale_05_size * 0.8, \
            f"Scale 2.0 ({scale_20_size:,} bytes) should be larger than scale 0.5 ({scale_05_size:,} bytes)"

    def test_expected_png_file_integrity(
        self,
        fixtures_expected_dir
    ):
        """Test that all expected PNG files are valid and non-empty."""
        png_files = list(fixtures_expected_dir.glob("*.png"))
        assert len(png_files) >= 11, f"Expected at least 11 PNG files, found {len(png_files)}"
        
        for png_file in png_files:
            # Check file is not empty
            assert png_file.stat().st_size > 1000, \
                f"Expected PNG file too small: {png_file} ({png_file.stat().st_size} bytes)"
    ...
    def test_svg_to_png_file_associations(
        ...
        generated_pngs = [png for png in expected_pngs 
                         if png.name not in basic_pngs]
